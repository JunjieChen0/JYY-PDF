  ...
  const handleAddPageNumbers = async () => {
    ...
    try {
      for (let i = 0; i < fileIds.length; i++) {
        ...
        const outputPath = await pdf.addPageNumbers(fileIds[i], {
          position,
          startNumber,
          fontSize,
          color,
      ...
      if (successCount > 0) {
        toast.success(`页码添加完成！成功处理 ${successCount}/${fileIds.length} 个文件`)
      }
    } catch (error) {
      if (error instanceof CancelledError) {
        toast.info(`操作已取消（已完成 ${successCount}/${fileIds.length} 个文件）`)
      } else {
        toast.error(`添加页码失败：${error instanceof Error ? error.message : String(error)}`)
      }
    } finally {
            ...
            {isProcessing && (
              ...
              >
                ...
                <p className="text-sm text-muted-foreground text-center">
                  正在添加页码... {selectedCount > 1 ? `(${currentFileIndex + 1}/${selectedCount})` : ''} {progress}%
                </p>
              </motion.div>
            )}
          </>
