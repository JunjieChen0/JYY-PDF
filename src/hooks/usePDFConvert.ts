...
import { assertUint8Array, getRequiredPdfData } from '@/lib/pdf-helpers'
...
export function usePDFConvert(files: PDFFile[]) {
  ...
  const convertToImages = useCallback(
    async (
      fileId: string,
      format: 'png' | 'jpg',
      ...
      try {
        for (let i = 1; i <= pages; i++) {
          ...
          if (!context) {
            throw new Error(t(ErrorCode.CANVAS_CONTEXT_FAILED))
          }
  ...
  const convertToText = useCallback(
    async (fileId: string, onProgress?: ProgressCallback, token?: CancellationToken) => {
      const file = filesRef.current.find((f) => f.id === fileId)
      if (!file) return null
  ...
  const imagesToPdf = useCallback(
    async (
      ...
      const firstImageName = imagePaths[0].replace(/\\/g, '/').split('/').pop() || 'images'
      const baseName = firstImageName.replace(/\.[^.]+$/, '')
      const result = await window.electronAPI.saveFile({
        defaultPath: `${baseName}_merged.pdf`,
      })
  ...
  const wordToPdf = useCallback(
    async (filePath: string, onProgress?: ProgressCallback, token?: CancellationToken) => {
      ...
      token?.throwIfCancelled()
      onProgress?.(60)
      const pdfResult = await window.electronAPI.convertWordToPdfData(wordData)
      if (pdfResult.error) throw new Error(pdfResult.error)
      if (!pdfResult.data) throw new Error(t(ErrorCode.WORD_RESULT_EMPTY))
